<?php
/**
 * 0503 통화내역 CSV 일회 반영 (linkconnect)
 *
...
/** 일회용 토큰 — 실행 후 스크립트/데이터 삭제 */
$expected_token = 'callimport-3929216501c0415637403569';
...
if ($action !== 'run' && !$is_cli) {
    echo json_encode(array(
        'ok' => true,
        'message' => 'action=run&token=... 으로 실행하세요. dryRun=1 미리보기, force=1 재실행.',
...
// 기존(2026-09-04) 일괄 임포트와 동일 providerCallId → 이미 있으면 중복 스킵
$rows = $parsed['rows'] ?? array();
foreach ($rows as $idx => $row) {
    $i = $idx + 1; // 헤더 다음 첫 데이터행 = 1 (당시 matrix $i)
    $virtual = (string) ($row['virtualNumber'] ?? '');
    $caller = (string) ($row['caller'] ?? '');
    $started = (string) ($row['startedAt'] ?? '');
    $duration = (string) ($row['duration'] ?? '');
    $rows[$idx]['providerCallId'] = 'import-20260904-' . $i . '-' . substr(md5($virtual . $caller . $started . $duration), 0, 10);
}
$parsed['rows'] = $rows;

// 현재 DB 건수 참고
$existing = 0;
if (function_exists('lc_db_installed') && lc_db_installed() && function_exists('lc_db_table_exists') && lc_db_table_exists(lc_table('call_logs'))) {
    $cnt = lc_sql_fetch(' SELECT COUNT(*) AS cnt FROM `' . lc_table('call_logs') . '` ');
    $existing = (int) ($cnt['cnt'] ?? 0);
}
...
$result['existingLogsBefore'] = $existing;
if (!empty($result['ok'])) {
