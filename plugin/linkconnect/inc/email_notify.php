...
if (!function_exists('lc_email_notify_send')) {
    function lc_email_notify_send($to, $subject, $html)
    {
        if (!lc_email_notify_mailer_ready()) {
            error_log('[LinkConnect EmailNotify] mailer unavailable');
            return false;
        }

        if (!lc_email_notify_board_enabled()) {
            error_log('[LinkConnect EmailNotify] cf_email_use is off');
            return false;
        }

        $to = trim((string) $to);
        if ($to === '' || !filter_var($to, FILTER_VALIDATE_EMAIL)) {
            error_log('[LinkConnect EmailNotify] invalid recipient');
            return false;
        }
        ...
        if ($from_email === '') {
            error_log('[LinkConnect EmailNotify] from email missing');
            return false;
        }
        ...
        try {
            ...
            $ok = (bool) $result;
            if (!$ok) {
                error_log('[LinkConnect EmailNotify] mailer returned empty for ' . $to);
            }
            return $ok;
        } catch (Throwable $e) {
...
if (!function_exists('lc_email_notify_dispatch_db_mode')) {
    function lc_email_notify_dispatch_db_mode($center, $user_id, $mode, $subject, $body_html, $digest_subject, $digest_body)
    {
        ...
        if ($mode === 'off') {
            error_log('[LinkConnect EmailNotify] skip ' . $center . '#' . $user_id . ' dbReceived=off');
            return false;
        }

        $recipient = lc_email_notify_resolve_recipient($center, $user_id);
        if ($recipient['email'] === '') {
            error_log('[LinkConnect EmailNotify] skip ' . $center . '#' . $user_id . ' no recipient email');
            return false;
        }
        ...
        if (empty($system['ready'])) {
            ...
            error_log('[LinkConnect EmailNotify] skip ' . $center . '#' . $user_id . ' ' . $issues);
            return false;
        }
        ...
        if (!$ok) {
            error_log('[LinkConnect EmailNotify] send failed ' . $center . '#' . $user_id . ' to=' . $recipient['email']);
        }
        return $ok;
    }
}
