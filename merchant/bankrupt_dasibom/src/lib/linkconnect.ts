const LK_CODE_STORAGE = 'lc_dasibom_lkCode';
...
export function buildInquiryText(fields: {
  ...
  const parts: string[] = [];
  if (fields.type) parts.push(`상담구분: ${fields.type}`);
  if (fields.time) parts.push(`희망시간: ${fields.time}`);
  if (fields.debt) parts.push(`채무금액: ${fields.debt}`);
  if (fields.income) parts.push(`월소득: ${fields.income}`);
  if (fields.status) parts.push(`현재상황: ${fields.status}`);
