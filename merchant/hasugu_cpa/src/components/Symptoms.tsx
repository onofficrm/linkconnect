...
const SYMPTOMS = [
  { id: 'slow', text: '물이 천천히 내려가요', icon: ArrowDownToLine, color: 'text-blue-500', bg: 'bg-blue-50' },
  { id: 'smell', text: '하수구에서 악취가 올라와요', icon: Wind, color: 'text-purple-500', bg: 'bg-purple-50' },
  { id: 'reverse', text: '물이 거꾸로 역류해요', icon: Waves, color: 'text-red-500', bg: 'bg-red-50' },
  { id: 'noise', text: '배관에서 이상한 소리가 나요', icon: Volume2, color: 'text-orange-500', bg: 'bg-orange-50' },
  { id: 'repeat', text: '뚫었는데 자꾸 다시 막혀요', icon: Repeat, color: 'text-amber-500', bg: 'bg-amber-50' },
  { id: 'sink_toilet', text: '싱크대 또는 변기가 막혔어요', icon: Bath, color: 'text-teal-500', bg: 'bg-teal-50' },
];
...
export default function Symptoms() {
  ...
  const handleSelect = (id: string) => {
    ...
    // In the future, this will sync with the form section
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-12 sm:mb-16">
          ...
          <p className="mt-4 text-slate-600 text-lg max-w-2xl mx-auto word-break-keep">
            해당하는 증상을 선택하시면 빠르고 정확한 상담이 가능합니다.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {SYMPTOMS.map((symptom, index) => {
            ...
            return (
              ...
              >
                <div className={`p-5 rounded-2xl mb-5 transition-colors duration-300 ${isSelected ? 'bg-blue-50' : symptom.bg} group-hover:scale-110`}>
                  <symptom.icon size={36} className={isSelected ? 'text-blue-600' : symptom.color} />
                </div>
                <h3 className={`text-lg sm:text-xl font-bold word-break-keep ${isSelected ? 'text-blue-700' : 'text-slate-800'}`}>
                  {symptom.text}
                </h3>
              </motion.button>
            );
